...
import { Budget, DraftBudgetSchema, ErrorResponseSchema, PasswordValidationSchema, RegisterSchema, SuccessSchema } from "@/src/schemas";
...
export async function deleteBudget(budgetId: Budget['id'], prevState: ActionStateType, formData: FormData) {
  ...
  if (!request.ok) {
    const error = ErrorResponseSchema.parse(deleteBudgetJson.error)
    return {
      errors: [error],
      success: ''
    }
